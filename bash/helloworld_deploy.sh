...
mkdir /tmp/buildDir/package -p

cd /tmp/buildDir

git clone https://github.com/warpedcodemonkey/helloworld.git

zip -r ./package/helloworld.zip helloworld

aws deploy push --application-name $2 --description "A deployment" --ignore-hidden-files --s3-location s3://$1/deploy-bundle.zip --source /tmp/buildDir/package/
